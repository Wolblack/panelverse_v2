...
async function loadItems(t,s){t=t||'';s=s||'';$('heading').textContent=t?t+' Archive':'Archive';$('subtitle').textContent='Search, filter, review and manage unified archive records.';var d=await api('/api/archive/items?type='+encodeURIComponent(t)+'&status='+encodeURIComponent(s));$('content').innerHTML='<div class="toolbar"><input id="search" class="input" placeholder="Search archive…" onkeydown="if(event.key===\\'Enter\\')loadItems()"><select id="status" class="select" onchange="loadItems(\\'\\',this.value)"><option value="">All statuses</option><option>DRAFT</option><option>REVIEW</option><option>APPROVED</option><option>PUBLISHED</option><option>ARCHIVED</option><option>REJECTED</option></select><button class="btn gold" onclick="openIngest()">＋ Add to Archive</button></div><div class="card list">'+(d.items.length?d.items.map(itemRow).join(''):'<div class="muted">No archive items match this view.</div>')+'</div>'}
