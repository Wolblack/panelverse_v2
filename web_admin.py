...
<script>
var type='BOOK', selectedFiles=[], activeXhrs={};
...
async function uploadFiles(){if(!selectedFiles.length)return toast('Select at least one file.');var box=$('files');box.style.display='block';box.innerHTML=selectedFiles.map(function(f,i){return '<div id="up-'+i+'" style="padding:10px 0;border-bottom:1px solid var(--line)"><b>'+esc(f.name)+'</b><button class="btn" style="float:right" onclick="cancelUpload('+i+')">Cancel</button><div class="muted">'+(f.size/1024/1024).toFixed(2)+' MB</div><div class="progress"><i id="bar-'+i+'" style="width:0%"></i></div><div id="state-'+i+'" class="muted">Queued</div></div>'}).join('');for(var i=0;i<selectedFiles.length;i++){await uploadOne(selectedFiles[i],i)}toast('Upload analysis jobs created');loadItems('','REVIEW')}
function uploadOne(file,index){return new Promise(function(resolve){var xhr=new XMLHttpRequest();var fd=new FormData();fd.append('content_type',type);fd.append('files',file,file.name);xhr.open('POST','/api/archive/ingest');xhr.withCredentials=true;activeXhrs[index]=xhr;xhr.upload.onprogress=function(e){if(e.lengthComputable){var pct=Math.round(e.loaded/e.total*100);$('bar-'+index).style.width=pct+'%';$('state-'+index).textContent='Uploading '+pct+'%'}};xhr.onload=function(){delete activeXhrs[index];if(xhr.status>=200&&xhr.status<300){$('bar-'+index).style.width='100%';$('state-'+index).textContent='Queued for analysis';resolve()}else{$('state-'+index).innerHTML='<span style="color:var(--bad)">Upload failed</span> <button class="btn" onclick="uploadOne(selectedFiles['+index+'],'+index+')">Retry</button>';resolve()}};xhr.onabort=function(){delete activeXhrs[index];$('state-'+index).textContent='Canceled'};xhr.onerror=function(){$('state-'+index).innerHTML='<span style="color:var(--bad)">Connection interrupted</span> <button class="btn" onclick="uploadOne(selectedFiles['+index+'],'+index+')">Retry</button>';resolve()};xhr.send(fd)})}
function cancelUpload(index){if(activeXhrs[index])activeXhrs[index].abort()}
function cancelAllUploads(){Object.keys(activeXhrs).forEach(function(i){activeXhrs[i].abort()})}
